...
## Exercise 16.3.1 - Google Cloud API & Services
...
## Exercise 16.3.2 OAuth Consent Screen
...
## Exercise 16.3.3 - Google API Credentials: Client Secret and Client Id
...
You're still missing the ``refreshToken``. The refreshToken is a requirement because of security reasons. In the world of APIs, tokens typically expire every 24 hours. So the ``refreshToken`` is needed to refresh the security token every 24 hours, so that your Source Connector-setup can keep connecting to Google Cloud Platfrom and BigQuery.


## Exercise 16.3.4 - BigQuery API and the refreshToken
...
If you use 2 Factor Authentication (2FA), enter the verification code sent to you.
...
After the 8 permisson-requests, you'll see this overview. Click ``Allow`` to finish the process.
...
Click the ``BigQuery``-card. Next, click on ``+ Connect source``.
...
![demo](./images/ex2/42.png)

You will now see the BigQuery-dataset you created during exercice 16.2. 
