...
Businesses have their own language for communicating about their domain. Car dealerships have makes, models, and cylinders. Airlines have  flight numbers, class of service, and seating assignments. Some of these terms are unique to a specific company, some are shared among an industry vertical, and some are shared by almost all businesses. For terms that are shared among an industry vertical or even broader, you can start doing powerful things with your data when you name and structure these terms in a common way.
...
* Create an XDM schema in the Data Collection interface
...
1. Ensure you are in the correct sandbox. Locate the sandbox in the upper right corner
   ...
   >[!NOTE]
   >
   >If you are the customer of a Platform application like Real-Time CDP or Journey Optimizer, we recommend using a new development sandbox for this tutorial. If you are not a customer of a Platform application, use the **[!UICONTROL Prod]** sandbox.
...
As noted earlier, XDM is the core framework that standardizes customer experience data by providing common structures and definitions. By adhering to XDM standards, _all customer experience data_ can be incorporated into a common representation. This approach allows you to gain valuable insights from customer actions, define customer audiences through segments, and create personalized messages with customer attributes using data from multiple sources. 

When possible, it is recommended to use existing field groups and adhere to semantic naming conventions. For any data specific to your organization that does not fit into the pre-defined field groups above, you can create a custom field group. See [Creating a schema using the Schema Editor](https://experienceleague.adobe.com/en/docs/experience-platform/xdm/tutorials/create-schema-ui#create) for more detailed steps on custom schemas.
