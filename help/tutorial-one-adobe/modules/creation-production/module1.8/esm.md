---
title: Enterprise Storage Management
description: Enterprise Storage Management
role: Developer
level: Beginner
jira: KT-5342
doc-type: Tutorial
---
# 1.8 Enterprise Storage Management

## Prerequisites

- ...

## Video

In this video, you'll get an explanation and demonstration of all the steps involved in this exercise.

>[!VIDEO](https://video.tv.adobe.com/v/3479582?quality=12&learn=on)

## Exercises

[1.8.1 Workfront & ESM](./ex1.md){target="_blank"}

In this exercise, you'll use Adobe Workfront in combination with Enterprise Storage Management.

[1.8.2 Frame.io & ESM](./ex2.md){target="_blank"}

In this exercise, you'll use Frame.io in combination with Enterprise Storage Management.

[1.8.3 Adobe Express & ESM](./ex3.md){target="_blank"}

In this exercise, you'll use Adobe Express in combination with Enterprise Storage Management.

![Tech Insiders](./../../../assets/images/techinsiders.png){width="50px" align="left"}

>[!NOTE]
>
>If you have questions, want to share general feedback of have suggestions on future content, please contact Tech Insiders directly, by sending an email to **techinsiders@adobe.com**.

Go back to [All Modules](../../../overview.md){target="_blank"}