...
Hover over the second **Set multiple variables** node and select **+** to add another module.
...
Enter the details for `x-api-key`. Select **Add**.
...
Enter the details for `Authorization`. Select **Add**.

![WF Fusion](./images/wffusion54.png)

Select **Add a header**. Enter the details for `Content-Type`. Select **Add**.

![WF Fusion](./images/wffusion541.png)

Select **Add a header**. Enter the details for `Accept`. Select **Add**.
...
Check the box for **Parse response**. Select **OK**.
...
Select the **?** icon on the fourth node, HTTP, to see the response. You should see an image file in the response.

![WF Fusion](./images/wffusion59.png)

Copy the image URL and open it in a browser window. Your screen should look like this:

![WF Fusion](./images/wffusion60.png)

Right-click **HTTP** and rename to **Firefly T2I**.
...
Select **wrench** between the nodes **Set Bearer Token** and **Firefly T2I**. Select **Add a router**.

![WF Fusion](./images/wffusion63.png)

Right-click **Firefly T2I** object and select **Clone**.

![WF Fusion](./images/wffusion64.png)

Drag and drop the cloned object close to the **Router** object—it auto-connects to the **Router**. Your screen should look like this:
...
Replace **Request content** by the below payload:
...
Make these 3 changes in the **inputs** section. Select **OK**.

![WF Fusion](./images/wffusion71.png)

Make these 3 changes in the **outputs** section. Select **OK**.
...
Select the **search** icon on the  **Photoshop Change Text** node to see the response. You should have a response that looks like this, with a link to a status file.

![WF Fusion](./images/wffusion76.png)

Before continuing with the Photoshop API interactions, disable the route to the **Firefly T2I** node to not send unneeded API calls to that API endpoint. Select the **wrench** icon, and then select **Disable route**.
...
Select the **Set multiple variables** node, select **Add item**. Select the variable value from the response of the previous request.
...
Select to open the HTTP request. Change the URL so that it references the variable that you created in the previous step, and set the **Method** to **GET**.
...
Select the **Tools** node and then select **Sleep**.

![WF Fusion](./images/wffusion90.png)

Position the **Sleep** node in between **Set multiple variables** and **Photoshop Check Status**. Set the **Delay** to **5** seconds. Select **OK**.
...
Select the **wrench** icon in between **Set multiple variables** and **Sleep**. Select **Add module**.
...
Set **Repeats** to **20**. Select **OK**.

![WF Fusion](./images/wffusion96.png)

Next, select **+** on the **Photoshop Check Status** to add another module.
...
Set **Source Module** to **Repeater**. Select **OK**.
...
Select the **wrench** icon and select **Add a module**.
...
Select **+ Add item** and set the **Variable name** to `done`.
...
Use `done` for the **Variable name**

To set the status, a boolean value is needed. To find the boolean value, select **gear** and then select `false`. Select **Add**.
...
Next, select the **wrench** icon after the **Get multiple variables** node that you configured.
...
For the label, use **Are we done?**. Set the **Condition** using the already existing variable **done**, the operator should be set to **Equal to** and the value should be the boolean variable `false`. Select **OK**.

![WF Fusion](./images/wffusion112.png)

Next, make some room between the nodes **Photoshop Check Status** and **Array aggregator**. Then, select the **wrench** icon and select **Add a router**. You're doing this because after checking the status of the Photoshop file, there should be 2 paths. If the status is `succeeded`, then the variable of **done** should be set to `true`. If the status is not equal to `succeeded`, then the loop should continue. The router will make it possible to check and set this.

![WF Fusion](./images/wffusion113.png)

After adding the router, select the **wrench** icon and select **Set up a filter**.

![WF Fusion](./images/wffusion114.png)

For the label, use **We are done**. Set the **Condition** using the response from the **Photoshop Check Status** node by choosing the resposne field **data.outputs[].status**, the operator should be set to **Equal to** and the value should be `succeeded`. Select **OK**.

![WF Fusion](./images/wffusion115.png)

Next, select the empty node with the question mark and search for **tools**. Then, select **Tools**.
...
For the **Variable name**, use `done`. 

For the **Variable value**, you should use the boolean value `true`. Select the **gear** icon and then select `true`. Select **Add**.
